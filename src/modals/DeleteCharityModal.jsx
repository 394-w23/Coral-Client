...
const DeleteCharityModal = ({ showModal, onCloseModal, capsuleData, slideOn }) => {
    const [update] = useDbUpdate(`/capsules/`);
    const capsuleCharity = capsuleData.goodwill.slice(1);

    const handleDelete = () => {
        const charityName = document.querySelector(`[id=charities]`)
        ...
        const request = document.querySelector(`[id=charities]`)
            .querySelector(`[data-active=true]`)
            .querySelector(`[id=charityCard]`)
            .querySelector(`[id=previewCard]`)
            .querySelector(`[id=cardContent]`)
        ...
        let updatedCharities = ["",];
            ...
            if (capsuleCharity[charity].charityName === charityName && capsuleCharity[charity].request === request) {
                continue;
            }
